        ...
        private void txtEntry_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keys.Enter)
                {
                    ...
                    ImageView.Image.Save(txtPath.Text + "\\result\\" + Path.GetFileName(locFile[counter]).Split('_')[0] + "_" + txtEntry.Text.Trim() + ".jpg");
                    ImageView.Image.Dispose();
                    File.Move(fd.SelectedPath + "\\" + Path.GetFileName(locFile[counter]), txtPath.Text + "\\" + Path.GetFileName(locFile[counter]).Split('_')[0] + "_" + txtEntry.Text.Trim() + ".jpg");
                    counter += 1;
                    showImage();
                    this.txtEntry.Clear();
                }
            ...
            try
            {
                switch (e.KeyData)
                {
                    ...
                    case Keys.Delete:
                        ...
                        break;
                    case Keys.F1:
                        ImageView.Image.Save(txtPath.Text + "\\result\\" + Path.GetFileName(locFile[counter]).Split('_')[0] + "_" + Path.GetFileName(locFile[counter]).Split('_').Last().Trim());
                        ImageView.Image.Dispose();
                        locFile = Directory.GetFiles(fd.SelectedPath, "*.jpg", SearchOption.TopDirectoryOnly).ToList();
                        counter += 1;
                        showImage();
